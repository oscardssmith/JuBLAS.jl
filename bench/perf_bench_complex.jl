...
const ELT   = length(ARGS) >= 3 ? Symbol(ARGS[3])     : :ComplexF32

BLAS.set_num_threads(1)

const T = ELT === :ComplexF64 ? ComplexF64 : ComplexF32
...
function bench(label, work)
    ...
    # Complex gemm: 8 real flops per output element per K (4 muls + 4 adds).
